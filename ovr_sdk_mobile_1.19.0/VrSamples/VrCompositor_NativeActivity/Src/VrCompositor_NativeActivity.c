	...
	const GLenum glInternalFormat, const void * data, const size_t dataSize, const bool mipSizeStored )
{
	...
	if ( numberOfFaces <= 1 )
	{
		chain = vrapi_CreateTextureSwapChain3( VRAPI_TEXTURE_TYPE_2D, GL_RGBA8, width, height, numStorageLevels, 1 );
	}
	else
	{
		chain = vrapi_CreateTextureSwapChain3( VRAPI_TEXTURE_TYPE_CUBE, GL_RGBA8, width, height, numStorageLevels, 1 );
	}
...
static void ovrScene_Create( ovrScene * scene )
{
	...
	// Simple checkerboard pattern.
	{
		...
		scene->EquirectSwapChain = vrapi_CreateTextureSwapChain3( VRAPI_TEXTURE_TYPE_2D,
			GL_RGBA8, width, height, 1, 1 );
	...
	// Simple checkerboard pattern.
	{
		...
		scene->CylinderSwapChain = vrapi_CreateTextureSwapChain3( VRAPI_TEXTURE_TYPE_2D,
			GL_RGBA8, scene->CylinderWidth, scene->CylinderHeight, 1, 1 );
	...
	for ( int i = 0; ; i++ )
	{
		...
		if ( cap.Type == ovrControllerType_Headset )
		{
			...
			if ( result == ovrSuccess )
			{
				...
				touchPadDownThisFrame |= headsetInputState.TrackpadStatus;
			}
		}
