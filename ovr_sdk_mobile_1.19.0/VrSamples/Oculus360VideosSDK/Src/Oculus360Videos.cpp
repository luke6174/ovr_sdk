...
void Oculus360Videos::EnteredVrMode( const ovrIntentType intentType, const char * intentFromPackage, const char * intentJSON, const char * intentURI )
{
	...
	if ( intentType == INTENT_LAUNCH )
	{
		...
		OVR_LOG( "--------------- Oculus360Videos OneTimeInit ---------------" );
	...
	OVR_LOG( "Oculus360Videos::EnteredVrMode" );
...
{
	OVR_LOG( "Oculus360Videos::LeavingVrMode" );
...
bool Oculus360Videos::IsVideoPlaying() const
{
	...
	if ( !methodId )
	{
		OVR_LOG( "Couldn't find isPlaying methodID" );
		return false;
	}
...
void Oculus360Videos::PauseVideo( bool const force )
{
	...
	if ( !methodId )
	{
		OVR_LOG( "Couldn't find pauseMovie methodID" );
		return;
	}
...
void Oculus360Videos::StopVideo()
{
	...
	if ( !methodId )
	{
		OVR_LOG( "Couldn't find stopMovie methodID" );
		return;
	}
...
void Oculus360Videos::ResumeVideo()
{
	...
	if ( !methodId )
	{
		OVR_LOG( "Couldn't find resumeMovie methodID" );
		return;
	}
...
void Oculus360Videos::StartVideo()
{
	if ( ActiveVideo )
	{
		...
		OVR_LOG( "StartVideo( %s )", ActiveVideo->Url.ToCStr() );
		...
		{
			OVR_LOG( "Couldn't find startMovie methodID" );
			return;
		}

		OVR_LOG( "moviePath = '%s'", ActiveVideo->Url.ToCStr() );
...
void Oculus360Videos::SetMenuState( const OvrMenuState state )
{
	...
	case MENU_VIDEO_RESUME:
		...
		break;
	default:
		OVR_LOG( "Oculus360Videos::SetMenuState unknown state: %d", static_cast< int >( state ) );
		OVR_ASSERT( false );
		break;
	}
}
