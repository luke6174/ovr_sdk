...
{
	OVR_LOG( "VideoBrowser::LoadThumbnail loading on %s", filename );
	...
	if ( orig )
	{
		...
		{
			OVR_LOG( "VideoBrowser::LoadThumbnail skip resize on %s", filename );
			return orig;
		}

		OVR_LOG( "VideoBrowser::LoadThumbnail resizing %s to %ix%i", filename, thumbWidth, thumbHeight );
	...
	{
		OVR_LOG( "Error: VideoBrowser::LoadThumbnail failed to load %s", filename );
	}
	return NULL;
}
