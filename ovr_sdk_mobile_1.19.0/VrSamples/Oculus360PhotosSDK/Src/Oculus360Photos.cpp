...
Oculus360Photos::~Oculus360Photos()
{
	...
	{
		OVR_FAIL( "eglDestroySurface: shutdown failed" );
	}
#endif
...
void Oculus360Photos::Configure( ovrSettings & settings )
{
	...
	UseSrgb = app->GetSystemProperty( VRAPI_SYS_PROP_SRGB_LAYER_SOURCE_AVAILABLE ) != 0;
	OVR_LOG( "Oculus360Photos::Configure : UseSrgb %d", UseSrgb );
...
void Oculus360Photos::EnteredVrMode( const ovrIntentType intentType, const char * intentFromPackage, const char * intentJSON, const char * intentURI )
{
	...
	if ( intentType == INTENT_LAUNCH )
	{
		...
		OVR_LOG( "--------------- Oculus360Photos OneTimeInit ---------------" );
		...
		if ( MetaData == NULL )
		{
			OVR_FAIL( "Oculus360Photos::OneTimeInit failed to create MetaData" );
		}
		...
		OVR_LOG( "360 PHOTOS using %d searchPaths", SearchPaths.GetSizeI() );
		...
		{
			OVR_FAIL( "Oculus360Photos::OneTimeInit - failed to access app cache storage" );
		}
		OVR_LOG( "Oculus360Photos::OneTimeInit found AppCacheStoragePath: %s", AppFileStoragePath.ToCStr() );
		...
		OVR_LOG( "META DATA INIT TIME: %f", SystemClock::GetTimeInSeconds() - startTime );
		...
		{
			OVR_FAIL( "eglChooseConfig failed" );
		}

		if ( EglConfig == NULL )
		{
			OVR_FAIL( "EglConfig NULL" );
		}
		if ( !eglQueryContext( EglDisplay, EglShareContext, EGL_CONTEXT_CLIENT_VERSION, ( EGLint * )&EglClientVersion ) )
		{
			OVR_FAIL( "eglQueryContext EGL_CONTEXT_CLIENT_VERSION failed" );
		}
		OVR_LOG( "Current EGL_CONTEXT_CLIENT_VERSION:%i", EglClientVersion );
		...
		if ( EglPbufferSurface == EGL_NO_SURFACE ) {
			OVR_FAIL( "eglCreatePbufferSurface failed: %s", GL_GetErrorString() );
		}
		...
		{
			OVR_FAIL( "eglQuerySurface failed:  %s", GL_GetErrorString() );
		}
#endif
	...
	OVR_LOG( "Oculus360Photos::EnteredVrMode" );
...
void * Oculus360Photos::BackgroundGLLoadThread( Thread * thread, void * v )
{
	...
	{
		OVR_FAIL( "eglCreateContext failed: %s", GL_GetErrorString() );
	}
	...
	{
		OVR_FAIL( "BackgroundGLLoadThread eglMakeCurrent failed: %s", GL_GetErrorString() );
	}
#endif
	...
	for ( ; ; )
	{
		...
		{
			OVR_LOG( "BackgroundGLLoadThread ShutdownRequest received" );
			break;
		}

		photos->BackgroundCommands.SleepUntilMessage();
		const char * msg = photos->BackgroundCommands.GetNextMessage();
		OVR_LOG( "BackgroundGLLoadThread Commands: %s", msg );
		if ( MatchesHead( "pano ", msg ) )
		{
			...
			{
				OVR_LOG( "Quartering oversize %ix%i image", width, height );
			...
			OVR_LOG( "%4.2fs to load %ix%i res pano map", end - start, width, height );
		}
		else if ( MatchesHead( "cube ", msg ) )
		{
			...
			const double end = SystemClock::GetTimeInSeconds();
			OVR_LOG( "%4.2fs to load %i res cube map", end - start, size );
		}
	}
	...
	{
		OVR_FAIL( "BackgroundGLLoadThread eglMakeCurrent: shutdown failed" );
	}

	if ( eglDestroyContext( photos->EglDisplay, EglBGLoaderContext ) == EGL_FALSE )
	{
		OVR_FAIL( "BackgroundGLLoadThread eglDestroyContext: shutdown failed" );
	}
#endif
	return NULL;
}
...
{
	OVR_LOG( "StartBackgroundPanoLoad( %s )", filename );
...
void Oculus360Photos::SetMenuState( const OvrMenuState state )
{
	...
	MenuState = state;
	OVR_LOG( "%s to %s", MenuStateString( lastState ), MenuStateString( MenuState ) );
	switch ( MenuState )
	{
	case MENU_NONE:
		break;
...
ovrFrameResult Oculus360Photos::Frame( const ovrFrameInput & vrFrame )
{
	...
	worldLayer.Header.Flags |= ( CurrentPanoIsCubeMap ) ? 0 : VRAPI_FRAME_LAYER_FLAG_CHROMATIC_ABERRATION_CORRECTION;
	worldLayer.Header.Flags |= ( UseSrgb ? 0 : VRAPI_FRAME_LAYER_FLAG_INHIBIT_SRGB_FRAMEBUFFER );
	worldLayer.Header.SrcBlend = ( CurrentPanoIsCubeMap ) ? VRAPI_FRAME_LAYER_BLEND_SRC_ALPHA : VRAPI_FRAME_LAYER_BLEND_ONE;
	worldLayer.Header.DstBlend = ( CurrentPanoIsCubeMap ) ? VRAPI_FRAME_LAYER_BLEND_ONE_MINUS_SRC_ALPHA : VRAPI_FRAME_LAYER_BLEND_ZERO;
