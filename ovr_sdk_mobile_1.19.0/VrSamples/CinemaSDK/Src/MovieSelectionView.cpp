...
void MovieSelectionView::OneTimeInit( const char * launchIntent )
{
	...
	OVR_LOG( "MovieSelectionView::OneTimeInit" );
	...
	OVR_LOG( "MovieSelectionView::OneTimeInit %3.1f seconds",  SystemClock::GetTimeInSeconds() - start );
}

void MovieSelectionView::OneTimeShutdown()
{
	OVR_LOG( "MovieSelectionView::OneTimeShutdown" );
}
...
{
	OVR_LOG( "SetMovieList: %zu movies", movies.GetSize() );
	...
	for ( UPInt i = 0; i < MovieList.GetSize(); i++ )
	{
		...
		OVR_LOG( "AddMovie: %s", movie->Filename.ToCStr() );
...
void MovieSelectionView::SetCategory( const MovieCategory category )
{
	...
	OVR_LOG( "SetCategory: %s", Categories[ categoryIndex ].Text.ToCStr() );
	...
	OVR_LOG( "%zu movies added", MovieList.GetSize() );
}
